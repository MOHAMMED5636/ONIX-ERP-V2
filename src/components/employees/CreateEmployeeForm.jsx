...
import { createEmployeeWithPhoto, apiRequest } from '../../services/authAPI';
...
function CreateEmployeeForm({ onSuccess, onCancel }) {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        const response = await apiRequest('/dashboard/projects');
        ...
        try {
          ...
          console.error('Failed to fetch projects from alternative endpoint:', e);
        }
      }
    };
    fetchProjects();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Object.keys(formData).forEach(key => {
        if (formData[key] !== null && formData[key] !== undefined && formData[key] !== '') {
          if (Array.isArray(formData[key])) {
            // Handle array fields (projectIds)
      ...
      const response = await createEmployeeWithPhoto(formDataToSend);
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 max-w-2xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <PhotoUpload onPhotoChange={setPhoto} size="md" />
              ...
              >
                {projects.map((project) => (
                  <option key={project.id} value={project.id}>
                    {project.referenceNumber || project.id} - {project.name}
                  </option>
                ))}
              </select>
              <p className="text-xs text-gray-500 mt-1">
